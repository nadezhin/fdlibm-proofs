...
(defund @cos-%0-bb (mem loc)
  ...
  (case (g '%9 loc)
    (-1 (mv '@cos-%10-bb mem loc))
    ( 0 (mv '@cos-%14-bb mem loc))
    (otherwise (mv nil mem loc)))))

(defund @cos-%10-bb (mem loc)
  (b* (
  ...
  (mv '@cos-%53-bb mem loc)))

(defund @cos-%14-bb (mem loc)
  (b* (
    (loc (s '%15 (load-i32 '(ix . 0) mem) loc))
    (loc (s '%16 (icmp-sge-i32 (g '%15 loc) 2146435072) loc)))
  (case (g '%16 loc)
    (-1 (mv '@cos-%17-bb mem loc))
    ( 0 (mv '@cos-%21-bb mem loc))
    (otherwise (mv nil mem loc)))))

(defund @cos-%17-bb (mem loc)
  (b* (
...
(defund @cos-%21-bb (mem loc)
  ...
  (case (g '%26 loc)
    (0 (mv '@cos-%27-bb mem loc))
    (1 (mv '@cos-%33-bb mem loc))
    (2 (mv '@cos-%40-bb mem loc))
    (otherwise (mv '@cos-%47-bb mem loc)))))

(defund @cos-%27-bb (mem loc)
  (b* (
...
(defund @cos-%40-bb (mem loc)
  (b* (
    (loc (s '%41 (getelementptr-double '(y . 0) 0) loc))
    (loc (s '%42 (load-double (g '%41 loc) mem) loc))
    (loc (s '%43 (getelementptr-double '(y . 0) 1) loc))
...
(defund @cos-%47-bb (mem loc)
  (b* (
    (loc (s '%48 (getelementptr-double '(y . 0) 0) loc))
    (loc (s '%49 (load-double (g '%48 loc) mem) loc))
    (loc (s '%50 (getelementptr-double '(y . 0) 1) loc))
    (loc (s '%51 (load-double (g '%50 loc) mem) loc))
    (loc (s '%52 (@__kernel_sin (g '%49 loc) (g '%51 loc) 1) loc))
    (mem (store-double (g '%52 loc) '(ret . 0) mem)))
  (mv '@cos-%53-bb mem loc)))

(defund @cos-%53-bb (mem loc)
  (b* (
    (loc (s '%54 (load-double '(ret . 0) mem) loc)))
  (mv 'ret mem loc)))

(defund @cos-step (label mem loc)
  (case label
    (%-0 (@cos-%0-bb mem loc))
    (%-10 (@cos-%10-bb mem loc))
    (%-14 (@cos-%14-bb mem loc))
    (%-17 (@cos-%17-bb mem loc))
    (%-21 (@cos-%21-bb mem loc))
    (%-27 (@cos-%27-bb mem loc))
    (%-33 (@cos-%33-bb mem loc))
    (%-40 (@cos-%40-bb mem loc))
    (%-47 (@cos-%47-bb mem loc))
    (%-53 (@cos-%53-bb mem loc))
    (otherwise (mv nil mem loc))))

(defund @cos-steps (label mem loc n)
  ...
  (if (equal label 'ret)
    ...
    (if (zp n) nil
      (mv-let
        (label mem loc)
        (@cos-step label mem loc)
        (@cos-steps label mem loc (1- n))))))
